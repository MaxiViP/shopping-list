<template>
	<div class="bg-white rounded-xl shadow">
		...
		<ul v-else class="max-h-96 overflow-y-auto overflow-x-hidden grid grid-cols-2 sm:grid-cols-3 gap-2 p-2">
			<li
				v-for="i in wishlist"
				:key="i.id"
				class="w-full p-2 sm:p-3 md:p-4 hover:bg-blue-50 flex justify-between items-center rounded break-words cursor-pointer"
			>
				<!-- Текст слева -->
				...
				<div class="flex gap-2 flex-shrink-0" @click.stop>
					...
					<!-- Сердечко всегда видно -->
					<button
						@click="$emit('toggle-favorite', i.id)"
						class="p-1 sm:p-2 md:p-2 rounded hover:bg-red-50 transition-colors"
						:title="favorites.includes(i.id) ? 'Удалить из избранного' : 'Добавить в избранное'"
					>
						<font-awesome-icon
							:icon="favorites.includes(i.id) ? 'heart' : 'heart'"
							:class="[
								'text-sm sm:text-base md:text-lg transition-colors',
								favorites.includes(i.id) ? 'text-red-500 hover:text-red-700' : 'text-gray-300 hover:text-red-400',
							]"
						/>
					</button>
				</div>
